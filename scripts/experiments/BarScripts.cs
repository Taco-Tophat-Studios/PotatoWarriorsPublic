...
public partial class BarScripts : Node2D
{
	...
	public override void _Ready()
	{
		...
		slider = (Node2D)GetNode("../SlidingPoint");
		drivingConnector = (Node2D)GetNode("../DrivingBar/ConnectionPoint");
		connector = (Node2D)GetNode("../ConnectionBar");
		c1 = (Node2D)GetNode("../OffsetBar/ConnectionPoint");
		c2 = (Node2D)GetNode("../JohnsonBar/ConnectionPoint");
		c = (Node2D)GetNode("../JohnsonBar/ConnectionPoint2");
		cThis = (Node2D)GetNode("ConnectionPoint"); 
	}

	// Called every frame. 'delta' is the elapsed time since the previous frame.
	public override void _Process(double delta)
	{
		//NOTE: make all of the Node2D initializers done on ready instead of every frame
		float radius = 48;
